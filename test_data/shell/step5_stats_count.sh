cd /datapool/test_data/00.rawdata/
...
perl /datapool/pipeline/metagenome/bin/sample_stats_extractor.pl /datapool/test_data/01.cleandata/*/*.json >/datapool/test_data/01.cleandata/sample.cleandata.stats
ls /datapool/test_data/01.cleandata/*/bowtie2.log|while read f;do grep 'overall alignment rate' $f|awk -v f=$f '{print f"\t"$1}' ;done |perl -ne 'chomp;@a=split/\t/,$_;$id=(split/\//,$a[0])[-2];print"$id\t$a[-1]\n"' >/datapool/test_data/01.cleandata/bowtie2.txt

cd /datapool/test_data/01.cleandata/kraken_count
perl /datapool/pipeline/metagenome/bin/merge.kraken2.pl /datapool/test_data/01.cleandata/*/*.label
...
cd /datapool/test_data/02.assembly/
...
cp /datapool/pipeline/metagenome/bin/plot_density.R /datapool/test_data/02.assembly/pick_1k_sequence/
cd /datapool/test_data/02.assembly/pick_1k_sequence/
